...
export default function PricingCalculator() {
  ...
  const calculatedPricing = useMemo(() => {
    ...
    const baseDataGB = currentTier.id === 'starter' ? 1000 : currentTier.id === 'professional' ? 10000 : 999999
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center space-y-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Pricing Calculator</h1>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">1. Select Your Company Profile</h3>
            ...
            >
              <div className="flex items-center space-x-3 mb-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-500 dark:text-slate-400">{profile.description}</div>
                </div>
              </div>
              ...
              {profile.id !== 'custom' ? (
                ...
                <div className="space-y-3">
                  <div className="space-y-2">
                    <label className="block text-xs font-medium text-gray-700 dark:text-slate-300">Team Size</label>
                    <input
                      type="number"
                      value={customTeamSize}
                      onChange={(e) => setCustomTeamSize(parseInt(e.target.value) || 1)}
                      className="w-full px-2 py-1 text-sm border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white rounded focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500"
                      min="1"
                      max="200"
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="block text-xs font-medium text-gray-700 dark:text-slate-300">Data Volume (GB/month)</label>
                    <input
                      type="number"
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">2. Choose Your Plan</h3>
          
          <div className="flex items-center space-x-3">
            <span className={clsx('text-sm', billingCycle === 'monthly' ? 'text-gray-900 dark:text-white' : 'text-gray-500 dark:text-slate-400')}>
              Monthly
            </span>
            <button
            ...
            <span className={clsx('text-sm', billingCycle === 'annual' ? 'text-gray-900 dark:text-white' : 'text-gray-500 dark:text-slate-400')}>
              Annual
            </span>
            {billingCycle === 'annual' && (
          ...
          {pricingTiers.map((tier) => (
            <div
              ...
              className={clsx(
                ...
                selectedTier === tier.id
                  ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/30 ring-2 ring-indigo-200 dark:ring-indigo-700 transform scale-105'
            ...
            >
              ...
              <div className="text-center mb-6">
                ...
                <p className="text-sm text-gray-600 dark:text-slate-300 mt-1">{tier.description}</p>
                
                <div className="mt-4">
                  <div className="text-3xl font-bold text-gray-900 dark:text-white">
                    ${tier.basePrice}
                    <span className="text-lg text-gray-500 dark:text-slate-400">/mo</span>
                  </div>
                  {billingCycle === 'annual' && (
                    <div className="text-sm text-green-600">
                      ${Math.round(tier.basePrice * 12 * 0.8)} billed annually
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-3 mb-6">
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div className="flex items-center space-x-2">
                    <UserGroupIcon className="h-4 w-4 text-gray-400 dark:text-slate-500" />
                    <span className="text-gray-900 dark:text-white">{tier.limits.users} users</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <CloudIcon className="h-4 w-4 text-gray-400 dark:text-slate-500" />
                    <span className="text-gray-900 dark:text-white">{tier.limits.storage}</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <ChartBarIcon className="h-4 w-4 text-gray-400 dark:text-slate-500" />
                    <span className="text-gray-900 dark:text-white">{tier.limits.searches}</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <SparklesIcon className="h-4 w-4 text-gray-400 dark:text-slate-500" />
                    <span className="text-gray-900 dark:text-white">{tier.limits.support}</span>
                  </div>
                </div>
              </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="text-center">
            ...
            <h4 className="font-semibold text-gray-900 dark:text-white mb-2">15x Faster</h4>
          ...
          <div className="text-center">
            ...
            <h4 className="font-semibold text-gray-900 dark:text-white mb-2">85% Cost Savings</h4>
            <p className="text-sm text-gray-600 dark:text-slate-300">
          ...
          <div className="text-center">
            ...
            <h4 className="font-semibold text-gray-900 dark:text-white mb-2">AV-Native</h4>
            <p className="text-sm text-gray-600 dark:text-slate-300">
