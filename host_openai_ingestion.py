#!/usr/bin/env python3
"""
Host-based OpenAI embedding generation script.
Runs directly on the Mac host to bypass Docker networking issues.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('host_openai_ingestion.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class HostOpenAIEmbeddingService:
    """OpenAI embedding service that runs on the host machine."""
                ...
                messages=[
                    {
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": """Analyze this traffic/driving scene image and provide a detailed description focusing on:

1. Vehicles present (cars, trucks, motorcycles, bicycles, buses, etc.)
2. Road infrastructure (highway, city street, intersection, bridge, etc.)
3. Traffic conditions (heavy, light, stopped, moving)
4. Weather and lighting conditions
5. Notable objects or scenarios (construction, accidents, pedestrians, etc.)

Be specific about vehicle types, road markings, signs, and any distinctive features that would help someone search for similar scenes."""
                            },
                            {
                                "type": "image_url",
                                "image_url": {
...
def get_database_connection():
    ...
    try:
        # Try connecting to Docker container database
...
def get_frames_without_embeddings(conn, batch_size: int = 5) -> List[dict]:
...
def update_frame_embedding(conn, frame_id: int, embedding: List[float], description: str):
    ...
    try:
        with conn.cursor() as cur:
            ...
            conn.commit()
            logger.info(f"Updated frame {frame_id} with OpenAI embedding")
...
def main():
    ...
    BATCH_SIZE = 3  # Conservative batch size to avoid rate limits
    DELAY_BETWEEN_BATCHES = 2  # seconds
    
    logger.info("Starting host-based OpenAI embedding generation")
    
    try:
        # Initialize services
        embedding_service = HostOpenAIEmbeddingService(OPENAI_API_KEY)
        ...
        while True:
            ...
            for frame in frames:
                try:
                    ...
                    # Database stores: /app/frames/filename.jpg
                    # Local path is: backend/frames/filename.jpg
                    ...
                    logger.info(f"Generating description for frame {frame['id']}")
                    description = embedding_service.create_frame_description(str(frame_path))
                    
                    # Generate embedding from description
                    logger.info(f"Generating embedding for frame {frame['id']}")
                    embedding = embedding_service.encode_text(description)
                    
                    # Update database
                    update_frame_embedding(conn, frame['id'], embedding, description)
                    
                    processed += 1
                    logger.info(f"Progress: {processed}/{total_frames} ({processed/total_frames*100:.1f}%)")
                    
                    # Small delay between individual frames
                    time.sleep(0.5)
            ...
            if frames:  # Only sleep if we processed frames
                logger.info(f"Batch complete. Sleeping {DELAY_BETWEEN_BATCHES}s...")
                time.sleep(DELAY_BETWEEN_BATCHES)
        
        logger.info(f"Embedding generation complete! Processed {processed} frames")
